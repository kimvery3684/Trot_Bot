...
# --- [1. 기본 설정 및 폴더 준비] ---
st.set_page_config(page_title="트로트 쇼츠 생성기 (매운맛)", page_icon="🔥", layout="wide")
...
QUIZ_TOPICS = [
    ...
    "통장에 현금 100억 쌓여있을 것 같은 재력왕은?",
    "부모님께 강남 아파트 일시불로 사드렸을 것 같은 효자는?",
    "친구에게 돈 1억, 묻지도 따지지도 않고 빌려줄 의리파는?",
    "밥값/술값 계산할 때 카드 가장 먼저 꺼낼 것 같은 사람은?",
    "나중에 1000억대 건물주 될 관상을 가진 사람은?",
    "보이스피싱 전화 오면 역으로 사기꾼 털어버릴 똑순이는?",

    # ❤️ 2. 며느리/사위 & 결혼 (댓글 전쟁 유발)
    "재벌가 사모님이 며느리/사위 삼고 싶어 안달 난 1위?",
    "상견례 프리패스상! 시어머니/장모님 사랑 독차지할 사람은?",
    "내 딸/아들과 결혼시키고 싶은 1등 신랑/신붓감은?",
    "결혼하면 배우자에게 아침밥 12첩 반상 차려줄 사랑꾼은?",
    "부부싸움 해도 먼저 애교부리며 사과할 것 같은 사람은?",
    "명절에 처가/시댁 가서 일 가장 잘 도울 것 같은 싹싹함 1위?",
    ...
    "관상학적으로 말년운이 가장 좋을 것 같은 사람은?",
    "성형외과 의사도 탐낼 완벽한 이목구비 1위는?",

    # 😡 4. 성격 & 반전 매력 (호기심 자극)
    "순하게 생겼는데 화나면 제일 무서울 것 같은 반전 성격은?",
    "학창시절 전교 1등 놓치지 않았을 것 같은 모범생은?",
    "무인도에 떨어져도 끝까지 살아남을 독종 생존왕은?",
    "거짓말 탐지기 해도 평온할 것 같은 강심장은?",
    "남의 말 절대 안 듣고 자기 고집대로 할 것 같은 '황소고집'은?",
    "눈물 많고 정 많아서 보증 잘못 서줄 것 같은 여린 사람은?",

    # 🎤 5. 끼 & 스타성
    "트로트 안 했으면 개그맨/배우 해서라도 성공했을 끼쟁이는?",
    "예능 프로그램 나가면 시청률 대박 날 입담꾼은?",
...
def generate_youtube_metadata(question, singers):
    titles = [
        f"🔥 {question} 1위는 과연 누구? #트로트",
        f"대박 결과 ㄷㄷ {question} 당신의 선택은? #{singers[0]} #{singers[1]}",
        f"※팬덤 전쟁※ {question} 솔직히 이분이죠? 😂",
        f"관상 테스트! {question} 소름돋는 결과",
        f"{singers[0]} vs {singers[1]}? {question} 승자는?"
    ]
    title = random.choice(titles)

    desc = f"""{question}

👇 여러분의 최애 가수를 댓글로 남겨주세요! 👇
...
🔥 매일 재밌는 트로트 랭킹이 올라옵니다! '구독'과 '좋아요' 부탁드려요!

#트로트 #미스터트롯 #현역가왕 #인기투표 #관상 #밸런스게임 #{singers[0]} #{singers[1]}
"""
    base_tags = "트로트, 트로트가수, 미스터트롯, 현역가왕, 미스트롯, 인기투표, 임영웅, 김호중, 이찬원, shorts, 쇼츠"
...
def generate_narration_script(question, singers):
    script = f"""(오프닝 - 흥미진진하게)
"팬 여러분, 솔직히 말해보세요! {question} 과연 누구일까요?"
...
(클로징 - 댓글 유도)
"와... 진짜 고르기 힘든데요?
여러분의 원픽을 지금 바로 댓글로 적어주세요!
좋아요 꾹!"
"""
    return script

# --- [6. 메인 UI] ---
st.title("🎵 트로트 쇼츠 생성기 (매운맛🔥)")
...
with st.sidebar:
    ...
    with tab_color:
        ...
        line_spacing = st.slider("📝 글자 줄간격 (행간)", 0, 100, 30)
